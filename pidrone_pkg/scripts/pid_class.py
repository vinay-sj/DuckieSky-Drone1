...
class PIDaxis():
    ...
    def step(self, err, time_elapsed):
        ...
        if self.i_range is not None:
            if self._i < self.i_range[0]:
                msg = "_i is too low (%f), setting to min range" % self._i 
                rospy.loginfo(msg)
            if self._i > self.i_range[1]:
                msg = "_i is too high (%f), setting to max range" % self._i
                rospy.loginfo(msg)
        if self.i_range is not None:
        ...
        new_output = raw_output + self.midpoint
        if new_output < self.control_range[0]:
            rospy.loginfo("command output too low (%f), setting to threshold", new_output)
        if new_output > self.control_range[1]:
            rospy.loginfo("command output too high (%f), setting to threshold", new_output)
...
class PID:
    ...
    def __init__(self,
                 ...
                 # Kv 2300 motors have midpoint 1300, Kv 2550 motors have midpoint 1250
                 throttle=PIDaxis(1.2, 0, 0, i_range=(-400,400), control_range=(1200,2000), midpoint=1400),
                 throttle_low=PIDaxis(1.2, 0, 0, i_range=(0,400), control_range=(1200,2000), midpoint=1400)):
"""
        ...
        self.roll_low.init_i = -1.1
        self.pitch_low.init_i = 0.9
        self.throttle_low.init_i = 105
        self.reset()
    ...
    def step(self, error, cmd_yaw_velocity=0):
        ...
        rospy.loginfo("z_error: %f", error.z)
        if abs(error.z) < self.trim_controller_thresh_throttle:
            ...
            if error.z > self.trim_controller_cap_throttle:
                rospy.loginfo("z error (%f) is too high, setting cap", error.z)
                self.throttle_low.step(self.trim_controller_cap_throttle, time_elapsed)
            elif error.z < -self.trim_controller_cap_throttle:
                rospy.loginfo("z error is too low (%f), setting to threshold", error.z)
                self.throttle_low.step(-self.trim_controller_cap_throttle, time_elapsed)
            else:
        ...
        rospy.loginfo("throttle cmd: %f ", cmd_t)
